...
const srcDir = 'images';
const outDir = path.join(srcDir, 'opt16x9');
...
async function walk(dir) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      if (full.startsWith(path.join(srcDir, 'opt')) || full.startsWith(path.join(srcDir, 'opt16x9'))) continue;
...
async function processFile(src) {
  for (const w of widths) {
    ...
    await ensureDir(dir);
    const p = (await sharp(src, { failOn: 'none' }).rotate()).resize({
      width: w, height: h, fit: 'cover', position: 'attention'
    });
    await p.clone().jpeg({ quality: qualityJpg, mozjpeg: true }).toFile(jpg);
    await p.clone().webp({ quality: qualityWebp }).toFile(webp);
    console.log('✓ crop16x9', src, '->', jpg, 'and', webp);
  }
}
